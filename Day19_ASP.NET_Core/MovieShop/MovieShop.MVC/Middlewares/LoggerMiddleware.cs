    ...
    public class LoggerMiddleware
    {
        ...
        public Task Invoke(HttpContext httpContext)
        {
            ...
            if (httpContext.User.Identity != null && httpContext.User.Identity.IsAuthenticated)
            {
                ...
                var fullname = httpContext?.User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.GivenName)?.Value + " " +
                               httpContext?.User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Surname)?.Value;

                var roles = httpContext?.User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Role)?.Value;
                ...
                _logger.LogInformation("Roles: {0} ", roles);
            }
